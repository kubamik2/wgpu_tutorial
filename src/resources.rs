...
pub fn load_texture(file_name: &str, device: &wgpu::Device, queue: &wgpu::Queue) -> anyhow::Result<crate::texture::Texture> {
    let data = load_binary(file_name)?;
    crate::texture::Texture::from_bytes(device, queue, &data, file_name)
}

pub fn load_model(file_name: &str, device: &wgpu::Device, queue: &wgpu::Queue, layout: &wgpu::BindGroupLayout) -> anyhow::Result<crate::model::Model> {
